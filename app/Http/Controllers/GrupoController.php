    ...
    public function agregaGrupo(Request $request)
{
    ...
    // Verificar si el usuario ya estÃ¡ dentro del grupo
    ...
    if ($usuario_en_grupo) {
        // El usuario ya estÃ¡ en el grupo, no es necesario agregarlo nuevamente
        return response()->json(['message' => 'El usuario ya estÃ¡ en el grupo'], 400);
    } else {
        // El usuario no estÃ¡ en el grupo, se puede agregar
        DB::table('tbl_grupos_user')->insert([
            'id_grupo' => $id_grupo,
            'id_user' => $id_user
        ]);
...
if ($usuario_en_grupo) {
    // Si el usuario está en el grupo, eliminar el registro
    DB::table('tbl_grupos_user')
        ->where('id_grupo', $id_grupo)
        ->where('id_user', $id_user)
        ->delete();
} else {
    // Si el usuario no está en el grupo, realizar alguna acción o mostrar un mensaje de error
    // por ejemplo, mostrar un mensaje con SweetAlert2
    echo "<script>Swal.fire('Error', 'El usuario no está en el grupo.', 'error');</script>";
}
